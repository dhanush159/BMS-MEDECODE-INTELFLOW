...
# import numpy as np
# import random
# words = ['jawbox', 'kvetch', 'flumps', 'drying','jumbly', 'dwarfs', 'kvetch', 'poxing','jawbox', 'fledgy', 'skrump', 'chintz']
# class ColorRect():
...
class ColorRect:
    ...
    def drawRect(
        ...
        # Put the text with adjusted font size for whiteBoard
        if self.text and self.text == "Board":
            text_size = cv2.getTextSize(
                self.text, fontFace, int(fontScale * text_scale), thickness
            )
        else:
...
# Create pen size buttons
pens = [
    ColorRect(1100, 50 + 100 * i, 100, 100, (50, 50, 50), str(penSize))
    for i, penSize in enumerate(range(5, 25, 5))
]
penBtn = ColorRect(1100, 0, 100, 50, color, "Pen")
...
while True:
        ...
        if upFingers[1] and not whiteBoard.isOver(x, y):
            ...
            if not hideColors:
                ...
                if clear.isOver(x, y):
                    clear.alpha = 0
                    canvas = np.zeros((720, 1280, 3), np.uint8)
                    # Display a random word from the words list
                    random_word = random.choice(words)
                    whiteBoard.setText(random_word)
                else:
                    clear.alpha = 0.5
    ...
    cv2.rectangle(
        frame,
        (colorsBtn.x, colorsBtn.y),
        (colorsBtn.x + colorsBtn.w, colorsBtn.y + colorsBtn.h),
        (255, 255, 255),
        2,
    )
    ...
    cv2.rectangle(
        frame,
        (boardBtn.x, boardBtn.y),
        (boardBtn.x + boardBtn.w, boardBtn.y + boardBtn.h),
        (255, 255, 255),
        2,
    )
